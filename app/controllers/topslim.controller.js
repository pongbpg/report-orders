            ...
            if (req.query.file != 'flash' && req.query.file != 'jt' && req.query.file != 'ninja') {
                ...
                orderx = orderx.map(order => {
                    if (order.name.substr(0, 1) == 'J') {
                        ...
                        return {
                            "เลขนำส่งพัสดุ": "",
                            "หมายเลขคำสั่งซื้ออีคอมเมิร์ซ": order.id,
    ...
    db.collection('orders')
        ...
        .then(snapShot => {
            ...
            snapShot.forEach(doc => {
                ...
                const covid = ((doc.data().product.filter(f => specials.indexOf(f.code) > -1 || f.typeId == 'EVENT').length > 0)
                ...
                orders.push({
                    ...
                    netTopslim: covid ? 0 : doc.data().price - (doc.data().totalFreight || 0),
                    netSpecial: covid ? doc.data().price - doc.data().totalFreight : 0,
                    netTotal: doc.data().price - (doc.data().totalFreight || 0),
        ...
        db.collection('orders')
            ...
            .then(snapShot => {
                ...
                db.collection('orders')
                    ...
                    .then(snapShot2 => {
                        snapShot2.forEach(doc2 => {
                            const xx = queryProvAmpr(doc2.data().addr);
